...
State Tracking monitors Claude CLI output to detect when sessions transition between 3 simplified states: WORKING → PENDING → IDLE. This enables real-time dashboard updates and targeted notifications.

## How It Works

### Current Implementation

**Output Monitoring** (`tools/tl-wrap.js`):
```javascript
let outputBuffer = '';
...
function processLine(line) {
  const cleanLine = line.replace(/\x1b\[[0-9;]*m/g, ''); // Remove ANSI
  
  // Current generic patterns (need Claude-specific updates)
  if (cleanLine.includes('Approve? [y/N]')) {
    updateTaskState('WAITING_USER', cleanLine);
  } else if (cleanLine.includes('Error:') || cleanLine.includes('error:')) {
    updateTaskState('ERROR', cleanLine);
  }
  // More patterns needed...
}
```
...
**✅ COMPLETED**: Ultra-simplified 3-state system
- ✅ WORKING state detection using "esc to interrupt" pattern
- ✅ PENDING state detection using "❯ 1. Yes" pattern
- ✅ IDLE as default state for everything else
- ✅ Backend and frontend updated for 3-state model
- ✅ Eliminated complex pattern matching and error detection